...
class FaissIndex:
    def __init__(self, index_file: str = "faces.index", dimension: int = 512):
        ...
        self.map_file = index_file.replace(".index", "_map.json")
        ...
        if os.path.exists(self.map_file):
            ...
            try:
                with open(self.map_file, 'r') as f:
                    ...
                    self.face_id_map = {int(k): v for k, v in loaded_map.items()}
            except Exception as e:
                print(f"Error loading face ID map: {e}")
    ...
    def save_index(self):
        ...
        try:
            with open(self.map_file, 'w') as f:
                json.dump(self.face_id_map, f)
        except Exception as e:
            print(f"Error saving face ID map: {e}")
